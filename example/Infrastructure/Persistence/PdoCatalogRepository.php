...
namespace Kavalhub\Example\Infrastructure\Persistence;
...
readonly class PdoCatalogRepository implements CatalogRepositoryInterface
{
    private PDO $pdo;
    ...
    {
        $sql = 'SELECT MIN(tpp.value) AS min_price,
                       MAX(tpp.value) AS max_price,
                       MAX(tpr.currency) AS currency
                FROM temp_product AS tp
                         LEFT JOIN temp_product_category AS tpc ON tpc.product_id = tp.id
                         LEFT JOIN temp_category AS tc ON tc.id = tpc.category_id
                         JOIN temp_product_facet AS tpf ON tpf.product_id = tp.id
